...
boomerang.controller('AboutControl', function ($scope, $http, $timeout, $location, $sce, Config) {
    ...
    var url = 'https://www.googleapis.com/plus/v1/people/' + Config.id + '?callback=JSON_CALLBACK&fields=aboutMe%2Ccover%2Cimage%2CplusOneCount%2Curls&key=' + Config.google_api
    ...
    $http.jsonp(url)
        .success(function (data) {
            ...
            for (i=0; i < data.urls.length; i++) {
                var url = data.urls[i];
                if (url.label.substring(0, 12) == 'Collaborator') {
                    var user = {
                        link: url.value
                    };
                    users.push(user);
...
boomerang.controller("NewsControl", function ($scope, $http, $timeout, $filter, $sce, Config) {
    ...
    var url = 'https://www.googleapis.com/plus/v1/people/' + Config.id + '/activities/public?callback=JSON_CALLBACK&maxResults=20&key=' + Config.google_api
              ...
              for (j = 0; j < attachments.length; j++) {
                  ...
                  switch (attachment.objectType) {
                      case 'album':
                          ...
                          for (k = 0; k < upper; k++) {
                              html.push('<li class="span2"><a href="' + attachment.thumbnails[k].url + '" target="_blank">' + '<img src="' + attachment.thumbnails[k].image.url + '" /></a></li>');
                          }
                          html.push('</ul>');
                          break;
